...
Phi_z1 = np.zeros(M)
for t in range(M):
    Phi_z1[t]= 2*np.pi*L**2*10**(-29)*integrate.simpson(B_z1[t][:],x=x[:])
    
np.savetxt(f'Phi_1_{t_end}_{M}_{Dead}.txt', Phi_z1)

Phi_z2 = np.zeros(M)
for t in range(M):
    Phi_z2[t]= 2*np.pi*L**2*10**(-29)*integrate.simpson(B_z2[t][:],x=x[:])
...
for t in range(M):
    Phi_z3[t]= 2*np.pi*2.25*10**(-3)*integrate.simpson(B_z3_i[t][:],x=x[:])
